...
- **Dark mode needs no `dark:` classes.** `tailwind.config.ts` points the pale backgrounds, the dark inks, the hairlines and `bg-white` at CSS variables that `html.dark` redefines, so plain `bg-white` / `text-gray-700` / `bg-red-50` already work in both themes. Only the families listed there are mapped. A colour family not on that list, an inline hex, or an SVG attribute stays the same in dark mode. The choice is per browser (localStorage, default light), set from the sidebar footer. See `src/lib/theme.ts`.
